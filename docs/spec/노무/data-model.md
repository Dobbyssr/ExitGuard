...
### 2-3. 항목별 `na`(해당없음) 유형규칙 — 접수 시 초기상태 결정 (계약 명문화) ★

코어 §4 상태머신의 `*→not_applicable`(유형 규칙, 접수 시 자동)은 **"어느 항목코드가 어느 `exit_reason`에서 대상 아님(na)이 되는가"** 를 규칙으로 갖는다. 코어 §4는 그 **메커니즘**(접수 시 자동 na 배치)을 정의하되 규칙 **내용**은 레일 지식이므로, 노무 레일이 여기서 규칙표로 고정한다. (규칙표가 계약에 없으면 구현자 재량으로 흔들려 데모 시드 `L-09=na`가 깨질 수 있음 — 명문화로 봉인.)

**MVP na 유형규칙 (freeze · 결정론)**

| 항목코드 | na가 되는 `exit_reason` 집합 | 근거 |
|---|---|---|
| `L-09` (해고예고 30일) | `{ voluntary, recommended_resignation, contract_expiry }` | 실제 '해고'(`dismissal`)가 아니면 해고예고 의무 자체가 없음(L1 §26 = 해고 시 조문). ∴ 해고 외 3유형은 대상 아님(na). |

```
def resolve_initial_status(exit_reason, code):        # 접수 시 각 Item 초기상태
    if exit_reason in NA_RULES.get(code, {}):  return not_applicable   # 유형상 대상 아님
    return pending                                                     # 그 외는 상신 대기
NA_RULES = { "L-09": {voluntary, recommended_resignation, contract_expiry} }
```

- **데모 정합**: 주인공 김민준 = `recommended_resignation` → `L-09 ∈ NA_RULES` → `not_applicable`(§3-3 표 · 코어 §4 freeze `L-09=not_applicable`와 일치). 게이트 `applicable(labor)`에서 L-09 제외(§3-3 파생).
- **경계(직역법 §4)**: na는 *유형상 해당 항목이 적용 대상이 아님*을 뜻하는 상태 표시일 뿐, 위법·적법의 판단이 아니다. `dismissal`(해고) 케이스에서는 L-09가 `pending`으로 살아나 서면통지(§27)·해고예고(§26) 대조 대상이 된다(exit_reason=`dismissal` 확장 예약 — §5-1 `dismissal_notice`).
- **[구현 후속 — 론] 정식 필드 승격(Post-MVP)**: 현 MVP는 규칙이 `L-09` 1건뿐이라 위 규칙표를 서비스 상수(`NA_RULES`)로 두는 것으로 충분하다(계약은 이 표가 진실). 레일별 규칙이 늘어나면 `TemplateItem`(코어 §3-6)에 **`na_when: list[ExitReason] | null`**(이 템플릿 항목이 na가 되는 사유유형 집합, 없으면 항상 pending) 필드로 승격해 데이터로 관리한다 — 규칙이 코드가 아닌 템플릿에 붙어 회사 커스텀 항목도 na 규칙을 가질 수 있게 된다. **이 필드 신설·마이그레이션은 론 코드 작업이며 이번 문서 정합 범위 밖**(규칙이 2건 이상 될 때 도비/헤르미온느 조율 후 착수).
...
| `decided_on` | date | ○ | CSV `작성일자`. **미보유 시 null 허용, CSV 실측 확보 시 채움**(순번 388 등 실측 사례에 작성일자 결여 — §4-2). `case_no` nullable과 동일 논리(가짜 날짜 창작 방지 — 스네이프 round4). |
| `views` | int | ○ | CSV `조회수` |
...
| **작성일자 일부 결여** | 실측 사례 일부(순번 388 등)에 `작성일자` 없음 | `decided_on` **nullable**(§4 표 ●→○ 정정). 미보유 시 null 유지·창작 금지, CSV 실측 확보 시 채움. risk/근거 서술은 작성일자 없이도 "…판정된 사례가 있습니다" 프레이밍으로 성립 |
...
> **구현 정합 (2026-07-21 · 도비 위임)**: ① `LaborPrecedent.decided_on` 필수(●)→선택(○) 정정 — 실측 결여(순번 388)로 구현 nullable, §4-2 한계표 정합(코드 `[결정필요]` 델타 해소). ② `na` 유형규칙 계약 명문화(§2-3 신설) — 구현 서비스 상수를 규칙표로 고정, 정식 필드(`TemplateItem.na_when`) 승격은 `[구현 후속 — 론]`. **신규 enum/shape/게이트 시드 재정의 0 — 정정·명시만.**
