...
- **타입 표기**: `str` `int` `bigint` `bool` `datetime`(UTC ISO-8601) `date` `Decimal` `enum` `FK→X` `list[X]` `json`.
  - **정수 폭 규약(freeze)**: **바이트 용량·대용량 정수는 반드시 64-bit(`bigint` / SQLAlchemy `BigInteger`)** 로 매핑한다. `int`는 int32(최대 ≈21억)로 오해될 수 있으므로 바이트류엔 쓰지 않는다. (근거: 데모 이상반출 4.2GB ≈ 45억 bytes가 int32를 초과 — `AnomalyExportLog.size_bytes` 등. 파일 첨부 바이트(`Approval.attachments[].size`)도 동일 원칙으로 64-bit 취급.)
- **필수여부**: ● 필수 / ○ 선택(nullable).
...
- **인덱스 원칙(계약 반영 · 구현은 론)**: PostgreSQL은 **FK에 인덱스를 자동 생성하지 않는다.** 따라서 **모든 FK 컬럼 + 지배적 조회 경로에 인덱스를 건다.** 특히 복합으로 — `Evidence(case_id, seq)`(체인 순서 조회 + §3-4-1 UNIQUE 겸함) · `Item(case_id, status)`(게이트 집계 §5·상태별 count §3-1-1 커버). 나머지 단일 FK(`Approval.item_id`·`Case.profile_id`·`AnomalyExportLog.case_id` 등)는 표준 단일 인덱스. 인덱스 정의(마이그레이션)는 론 몫이나 **"FK+지배적 조회 경로에 인덱스"는 계약 원칙**이다.
- **삭제·보존 정책(계약 · freeze)**: 봉인 데이터의 물리 소실을 금지한다.
  - **`Evidence`는 물리 삭제 금지**(append-only 계약 §3-4의 연장). Case가 사라져도 봉인 증적은 보존된다.
  - **`ON DELETE CASCADE` 금지 대상**: `Evidence`(및 봉인 대상 로그) · `Approval`(승인 이력). Case 삭제가 봉인 증거·승인 이력을 연쇄 삭제하면 제품 핵심 가치(위변조 탐지 무결성)가 붕괴한다.
  - **Case는 물리 삭제 대신 soft-delete(상태 플래그) 또는 삭제 자체를 막는다.** MVP는 Case 삭제 유스케이스가 없으므로 기본 `NO ACTION`(FK 위반으로 삭제 차단)만으로도 증적 보호가 성립 — soft-delete 컬럼 도입은 삭제 UI가 생길 때(Post-MVP). 어느 쪽이든 **증적은 어떤 삭제 경로로도 소실되지 않는다**가 불변 계약이다.

> **[Post-MVP · 구현 권고 노트 · 계약 강제 아님]** 아래는 지금 구현을 막지 않는 론(백엔드) 재량 권고다. MVP 착수는 현 계약대로 진행하되, 다음을 권고한다 — ① **enum은 native PG ENUM 대신 `VARCHAR + CHECK`**(SQLAlchemy `native_enum=False`): 값 추가가 CHECK 재생성만으로 끝나 확장 예약(`ItemKind.external`·레일 추가·`LaborRequiredElement` 확장)에 유리. ② **json 컬럼은 `jsonb`**(연산·인덱싱 우월). 단 `Evidence.payload` 해시 입력은 **봉인 시점 canonical_json 문자열 자체**를 해시하고 저장값에서 재직렬화해 해시하지 않는다(§3-4-1 재현성 규약 — 저장이 json/jsonb든 무관). ③ **게이트 목록(CM-03) 집계는 케이스별 반복(N+1) 금지, `Item`을 `GROUP BY case_id, status` 1쿼리로**(위 `Item(case_id, status)` 인덱스가 커버). ④ PK 타입(int vs uuid)·`ON DELETE` 세부·배열 컬럼(PG array vs jsonb)은 론 재량.
...
| `signed` | bool | ● | 전자서명 여부 |
| `basis_note` | str | ○ | **검토 시점 근거·사유 메모**(관리자 검토가 기록). 확인 시=확인 근거(예: "근로기준법 §36 금품청산 14일 이내"), 반려 시=반려 사유. §3-3-1 이중용도 규약. |
...
#### 3-3-1. `basis_note` 이중용도 규약 (검토 메모 저장 위치 명시) ★

검토(review) 시 관리자가 남기는 **확인 근거/반려 사유 메모**는 별도 컬럼을 신설하지 않고 **`basis_note` 한 필드에 기록**한다(신설 없이 문서화 — 구현 정합).

- **기록 시점 = 검토(review)뿐.** 상신(submit)은 `memo`(상신자 메모)만 쓰고 `basis_note`는 건드리지 않는다. 즉 `basis_note`의 **유일 기록자는 검토 단계**이므로 "기준 근거 문구"와 "검토 메모"가 같은 필드에 담겨도 실제 값 충돌이 없다(상신자 `memo`와 역할 분리 — memo=상신 근거, basis_note=검토 근거/사유).
- **의미 매핑**: `decision=confirmed` → `basis_note` = 확인 근거(기준 인용 형태 가능). `decision=rejected` → `basis_note` = 반려 사유.
- **증적 봉인 정합(§3-4-1)**: `item_confirmed` payload의 "기준 근거 문구(basis_note)" = 이 필드. `item_rejected` payload의 "반려 사유" = 같은 `basis_note` 값(봉인 payload에선 `reason` 키로 표기 — 라벨만 다름, 원천은 동일 필드). api-spec §2-4 review의 `memo`가 이 필드로 저장된다.
- **[Post-MVP · 분리 시]** 상신 시점 기준-근거와 검토 시점 사유를 **동시에** 별도 보관해야 하는 요건이 생기면 그때 `review_note`(검토 전용) 컬럼을 신설해 분리한다 — MVP는 두 값이 공존하지 않아 `basis_note` 단일 필드로 충분(YAGNI). 이 분리는 론 코드 작업이며 현 정합 범위 밖.
...
- **변경불가**: 생성 후 수정 불가(append-only). `integrity_hash` = SHA-256(payload), `prev_hash` 체인으로 이력 위변조 탐지. **물리 삭제도 금지**(§0 삭제·보존 정책) — Case 삭제·CASCADE로도 봉인 증적은 소실되지 않는다.
...
- **append·해시 체인 규약(freeze)**:
  ...
  - **채번 동시성 보장(freeze)**: 위 `seq` 채번은 Read-Modify-Write이므로 같은 case에 처리 이벤트가 동시 도착하면 seq 중복·체인 분기(두 레코드가 같은 prev_hash를 가리켜 단일 체인이 깨짐)가 발생할 수 있다. 이를 계약 레벨에서 막는다:
    1. **`UNIQUE(case_id, seq)` 제약**(Evidence 필수). 동시 append의 "조용한 체인 분기"를 "시끄러운 IntegrityError"로 바꿔 무결성 위반을 즉시 드러낸다.
    2. **채번 직렬화** — 봉인 트랜잭션 시작 시 해당 **Case 행을 `SELECT … FOR UPDATE`로 잠그고** 그 안에서 `max(seq)+1`을 채번한다(Case당 잠금이라 케이스 간 병렬성 손해 없음). 이로써 "seq 단조증가"가 동시성 하에서도 보장된다.
    - 위 원자성(봉인=같은 트랜잭션)은 요청=단일 트랜잭션 경계가 이미 보장한다 — 봉인에 별도 커밋을 끼워 이 원자성을 깨지 말 것. (§10 `head_hash` 체인 앵커가 단일 체인으로 유지되는 근거.)
...
> **`case_id` 없음 → 독립 `/compare`의 `unmet_count` 처리 규약(freeze)**: `CompareInput`은 케이스에 연계되지 않는다(공용 대조 shape — CM-05/LB-04/TS-05 공통). 따라서 **독립 `POST /compare`(케이스 비연계) 호출은 실제 케이스의 미충족 건수를 알 수 없어 `unmet_count = 0`** 으로 반환하고, `status` 행은 "케이스와 연계하면 미충족 건수·기한이 함께 제공됩니다" 안내로 채운다. **케이스 연계 실측치(`unmet_count`·D-day)는 `POST /cases/{id}/intake-compare`(래퍼)가** 케이스 Item을 롤업(§노무 §3-2)해 채운다. 즉 `unmet_count`의 실측 롤업은 intake-compare 경로에서만 성립하며, 이는 창작이 아니라 shape에 case 연계가 없다는 계약의 필연적 귀결이다(api-spec §2-3).
...
### 9-R. DB 공학 감수 반영 (2026-07-20 · 스네이프 round1 · 대표님 승인)

개발 착수 전 DB 무결성/위생 계약 보강. **기존 enum/shape/필드/게이트 시드 재정의 0건 — 제약·타입 폭·인덱스·정책만 추가·명시.** 정수 절 번호 불변(§0 규약 bullet·§3-4-1 소절 규약·§3-4 bullet 확장으로 삽입). 감수 리포트: `qa/db-review/2026-07-20-round1-계약+인프라.md`.

- **(k) 🔴-2 바이트 타입 BIGINT(§0 타입 범례 + 보안 data-model)**: `AnomalyExportLog.size_bytes` `int`→`bigint`. 데모 4.2GB(≈45억)가 int32(21억) 초과 → 시드 파손 방지. 코어 §0에 "바이트/대용량 정수는 64-bit(BIGINT)" 정수 폭 규약 명시(구현자 int32 오해 차단). API 응답은 `size_label`(문자열)만 노출 → **api-spec 무파급**.
- **(l) 🔴-1 증적 seq 채번 동시성(§3-4-1)**: `UNIQUE(case_id, seq)` 제약 + 채번 시 Case 행 `SELECT … FOR UPDATE` 잠금으로 seq 직렬화. 기존 "seq 단조증가·prev_hash 체인" 서술 유지, **동시성 보장 수단만 추가**(계약이 "단조"만 말하고 수단이 없던 공백 메움). 봉인=단일 트랜잭션 원자성은 기존 경계가 보장(참조).
- **(m) 🔴-3 인덱스 원칙(§0 규약)**: "FK 컬럼 + 지배적 조회 경로(예: `Evidence(case_id, seq)`·`Item(case_id, status)`)에 인덱스" 한 줄. PostgreSQL은 FK 인덱스 자동 생성 안 함 — 계약이 원칙 명시, 인덱스 정의는 론.
- **(n) 증적 soft-delete·보존 정책(§0 규약 + §3-4 bullet)**: `Evidence` 물리 삭제 금지(append-only 연장), Case 삭제·`ON DELETE CASCADE`로도 증적/승인 이력 소실 금지. Case는 soft-delete 또는 삭제 차단. ([결정필요]였던 삭제 정책을 스네이프 권고대로 "증적 물리 삭제 불가"로 확정.)
- **(o) [Post-MVP 노트 · 계약 강제 아님] §0 callout**: enum `VARCHAR+CHECK`(native ENUM 대신)·json→jsonb·게이트 목록 N+1(GROUP BY 1쿼리)·PK 타입 재량을 **구현 권고로만** 기록 — 지금 구현 안 막음. **제외**: 테스트 DB `_test` 가드(🟡)는 `backend/app/config.py` = 론 인프라 영역 → 계약 보강 대상 아님(도비가 론에 별도 인계).

### 9-S. 구현 정합 (2026-07-21 · 구현정합 5건 · 도비 위임)

첫 도메인(노무 세로절단) 구현 중 드러난 계약↔구현 불일치를 정합. **기존 enum/shape/게이트 시드/정수 절번호 재정의 0건 — 추가·명시·정정만.** 코어 data-model에 파급된 것은 아래 2건(나머지 3건은 api-spec §5·노무 data-model 델타).

- **(p) `Approval.basis_note` 이중용도 규약(§3-3-1 신설 + §3-3 필드 설명 명시)**: 검토(review) 시 확인근거/반려사유 메모를 별도 컬럼 신설 없이 `basis_note`에 기록함을 문서화. 구현상 `basis_note`의 유일 기록자가 검토 단계라 상신 `memo`와 실제 충돌 없음(§3-4-1 증적 payload 정합 유지). 전용 `review_note` 분리는 Post-MVP(YAGNI · 론 후속). 신설 필드 0.
- **(q) 독립 `/compare` `unmet_count=0` 규약(§6-1 주석 신설)**: `CompareInput`에 `case_id`가 없어 독립 `POST /compare`는 케이스 미충족 건수를 낼 수 없음 → `unmet_count=0` + 케이스 연계 안내. 실측 롤업은 `intake-compare`가 담당(현 구현 동작 문서화, shape 불변). api-spec §2-3 동시 갱신.
